			  ...
			  ParamListInfo boundParams, AQPState * aqp_state)
{
     ...
     * AQP
     */
	plan = planner(querytree, query_string, cursorOptions, boundParams, aqp_state);
 ...
 * AQP - FUNCTION
 *
 * AQP_exec_simple_query
 *
 * Execute a "simple Query" protocol message.
 */
static void
AQP_exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		/*
		 * Get the command name for use in
		 * status display (it also becomes the default completion tag, down
		 * inside PortalRun).  Set ps_status and do any special
        ...
        /* Alloc context for every query */
		if (per_parsetree_context == NULL)
		{
		...
		while (!aqp_state->need_break)
		{
			 ...
			 */
			if (aqp_state->version == 1 && analyze_requires_snapshot(parsetree))
			{
			...
			/*
			 * Switch to the context prepared for physical optimization
			 */
			oldcontext = MemoryContextSwitchTo(per_parsetree_context);

            /*
             * Only need to generate the Query in the first time
             */
			if (aqp_state->version == 1)
			{
				querytree_list = pg_analyze_and_rewrite(parsetree, query_string,
														NULL, 0, NULL);
                /*
                 * Judge whether to open AQP
                 */
				aqp_state->need_break = AQPNeedBreak(querytree_list, aqp_state);
			}

			plantree_list = pg_plan_queries(querytree_list, query_string,
											CURSOR_OPT_PARALLEL_OK, NULL, aqp_state);

			/*
			 * Done with the snapshot used for parsing/planning.
			 *
			 * While it looks promising to reuse the same snapshot for query
			 * execution (at least for simple protocol), unfortunately it
			 * causes execution to use a snapshot that has been acquired
			 * before locking any of the tables mentioned in the query.  This
			 * creates user- visible anomalies, so refrain.  Refer to
			 * https://postgr.es/m/flat/5075D8DF.6050500@fuzzy.cz for details.
			 */

            /* Pop snapshot in the final stage */
			/* TODO: (Zackery) Maybe have some problems in mvcc */
			if (aqp_state->need_break && snapshot_set)
			{
				PopActiveSnapshot();
			}
			 ...
			 * everything we are passing here is in MessageContext or the
			...
			if (aqp_state->need_break && aqp_state->version == 1)
				...
				AQP_PortalStart(portal, NULL, 0, InvalidSnapshot, aqp_state);
			...
			{
                /* TODO: (Zackery) Support to close all relations in the last time*/
				...
				AQP_ExecutorEnd(portal->queryDesc);
	...
	/* close hook */
	SetPlannerHook(false);
	SetJoinSearchHook(false);
}
			...
			case 'Q':			/* simple query */
				{
                    ...
                    if (open_aqp)
                    {
                        AQP_exec_simple_query(query_string);
                    }
                    else
                    {
                        if (am_walsender)
